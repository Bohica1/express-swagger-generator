...
function generateSpecAndMount(app) {
    ...
    return async function (options) {
		...
		let filePath = options.files ? options.files[0] : path.join(__dirname, '../../dist/api/endpoints/public/docs.json')
		await fsAsync.writeFile(filePath, JSON.stringify(swaggerObject))
